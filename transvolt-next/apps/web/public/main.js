...
/**
 * Interactive 3-Way Theme Toggle
 */
function setupThemeToggle() {
  const selectorContainer = document.getElementById("themeSelectorContainer");
  ...
  function applyTheme(theme) {
    ...
    if (headerLogo) {
      headerLogo.src = (theme === "dark" || theme === "black") ? "./Logo & Color/SVG/Logo_White.svg" : "./Logo & Color/SVG/Logo_Black.svg";
    }
  }

  applyTheme(currentTheme);

  if (themeDots.length > 0 && selectorContainer) {
    themeDots.forEach(dot => {
      dot.addEventListener("click", (e) => {
        ...
        let themeName = "Light";
        if (theme === "dark") themeName = "Navy Blue";
        if (theme === "black") themeName = "Full Black";
        showToast(`Switched to ${themeName} Theme`, "info");
        
        // Smoothly close the widget
        selectorContainer.classList.add("force-close");
        setTimeout(() => {
          selectorContainer.classList.remove("force-close");
        }, 500);
      });
    });
  }
...
function setupMobileMenu() {
  ...
  if (hamburgerBtn && navTabsContainer) {
  ...
  globalReplaceInput.accept = "image/*";
    ...
    if (e.target.files && e.target.files.length > 0) {
      ...
      if (currentReplaceTarget) {
        ...
        reader.onload = (event) => {
          ...
          // Case 1: Logo-box-item or lh-asset-box
          const assetBox = currentReplaceTarget.closest('.logo-box-item, .lh-asset-box');
          if (assetBox) {
            const img = assetBox.querySelector('.logo-preview-img, img');
            if (img) img.src = newUrl;
          }
          
          // Case 2: Photo gallery box
          const photoBox = currentReplaceTarget.closest('.photo-preview-box');
          if (photoBox) {
            const bg = photoBox.querySelector('.photo-bg');
            if (bg) {
              bg.style.backgroundImage = `url('${newUrl}')`;
            } else {
              // Letterhead boxes don't have .photo-bg child elements
              photoBox.style.background = `url('${newUrl}') center/cover no-repeat`;
            }
            photoBox.dataset.img = newUrl; // update for lightbox preview
          }
  ...
  document.addEventListener("click", (e) => {
    ...
    if (replaceBtn) {
      ...
      currentReplaceTarget = replaceBtn;
